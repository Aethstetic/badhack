...
public:
	void* operator new(size_t iAllocSize)
	{
		...
		if (!pKeyValuesSystem)
		{
			...
			oKeyValuesSystem KeyValuesSystem = (oKeyValuesSystem)GetProcAddress(GetModuleHandle("vstdlib.dll"), "KeyValuesSystem");
			pKeyValuesSystem = KeyValuesSystem();
		}

		typedef PVOID(__thiscall* oAllocKeyValuesMemory)(PVOID, size_t);
		return g_Utils->get_vfunc<oAllocKeyValuesMemory>(pKeyValuesSystem, 1)(pKeyValuesSystem, iAllocSize);
	}
	...
	void operator delete(void* pMem)
	{
		...
		g_Utils->get_vfunc<oFreeKeyValuesMemory>(pKeyValuesSystem, 2)(pKeyValuesSystem, pMem);
	}
...
public:
	...
	NetChannel* GetNetChannel()
	{
		...
		return g_Utils->get_vfunc<Fn>(this, 78)(this);
	}
};